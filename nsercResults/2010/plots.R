...
#  pdf(paste(province, ".pdf", sep=""),width=15)
  ...
  p + geom_point(aes(color=Institution)) +
    geom_text(hjust=0, vjust=0,aes(size=0.5),angle=90)+
      annotate("text", x=3.2, y=5000, label="Prepared by Daniel German dmg@uvic.ca",size=3,hjust=0, vjust=0) +
    opts(title=paste(province, " NSERC Discovery Grants 2010",sep=""))+   scale_x_continuous("Individuals") +   scale_y_continuous("Amount")
#  dev.off()
}
...
x<-read.csv("nserc2010.csv", sep=";")
nserc<-x
#join province
prov<-read.csv("../2011/byProvince.csv",sep=';')
...
pdf("on2010.pdf",width=15)
cg(disc2, "Ontario")
dev.off()

pdf("bc2010.pdf",width=15)
cg(disc2, "British Columbia")
dev.off()

pdf("qc2010.pdf",width=15)
cg(disc2, "Quebec")
dev.off()

pdf("ab2010.pdf",width=15)
cg(disc2, "Alberta")
dev.off()


pdf("nserc2010byInd.pdf",width=15,heigh=7)
...
pdf("nserc2010byInstLabels.pdf",width=15,heigh=7)
...
p+geom_hline(yintercept=24000,linetype=2,size=0.25)+
  ...
  opts(axis.text.x=theme_text(angle=-90, hjust=0),title="NSERC Discovery Grants 2010, Computer Science (Evaluation Group 1507)")+
  geom_vline(xintercept = c(8.5,10.5,11.5,12.5,14.5,32.5,33.5,43.5),color='black',alpha=0.5) +
  annotate("text", x=3.2, y=122000, label="Prepared by Daniel German dmg@uvic.ca",size=3) +
  geom_text(hjust=0, vjust=0,aes(size=amount),angle=45)+
  scale_x_discrete("Institution (ordered by Province)") +
  scale_y_continuous("Amount")
dev.off()

pdf("nserc2010byInst.pdf",width=15)
p<-ggplot(disc,aes(inst, amount,label=Name))
p+geom_hline(yintercept=24000,linetype=2,size=0.25)+
  geom_boxplot(color="black",outlier.size=0.1) +
  geom_jitter(aes(color=Term),size=2.5,alpha=0.7)+
