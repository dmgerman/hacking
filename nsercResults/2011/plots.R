...
#  pdf(paste(province, ".pdf", sep=""),width=15)
  ...
  p + geom_point(aes(color=Institution)) +
    ...
    opts(title=paste(province, " NSERC Discovery Grants 2011",sep=""))+   scale_x_continuous("Individuals") +   scale_y_continuous("Amount")
#  dev.off()
}
...
pdf("on2011.pdf",width=15)
cg(disc2, "Ontario")
dev.off()

pdf("bc2011.pdf",width=15)
cg(disc2, "British Columbia")
dev.off()

pdf("qc2011.pdf",width=15)
cg(disc2, "Quebec")
dev.off()

pdf("ab2011.pdf",width=15)
cg(disc2, "Alberta")
dev.off()
...
pdf("nserc2011byInd.pdf",width=15,heigh=7)
...
pdf("nserc2011byInstLabels.pdf",width=15,heigh=7)
...
p+geom_hline(yintercept=24000,linetype=2,size=0.25)+
  ...
  opts(axis.text.x=theme_text(angle=-90, hjust=0),title="NSERC Discovery Grants 2011, Computer Science (Evaluation Group 1507)")+
...
pdf("nserc2011byInst.pdf",width=15)
p<-ggplot(disc,aes(inst, amount,label=Name))
p+geom_hline(yintercept=24000,linetype=2,size=0.25)+
  geom_boxplot(color="black",outlier.size=0.1) +
  geom_jitter(aes(color=Term),size=2.5,alpha=0.7)+
