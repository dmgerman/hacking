...
# requires the output of printClones that has been sorted 

use strict;
require "simplifyFuncs.pm";
...
while (<>) {
    ...
    die unless $c->{toLen} = $c->{fromLen};
#    print "$c->{fromFile}, $c->{toFile}, $c->{fromBeginToken}, $fromLen,  $c->{toBeginToken}, $toLen\n";

    # we hit a new file, hence we print it
...
#            print "Done\n";
            Report_Cloned_Sections($prevFromFile, \@fromSections);
            print ":-1:";
            Report_Cloned_Sections($prevToFile, \@toSections);
            print "\n";
...
#            Report($prevFromFile,$prevToFile,$currentBegin, $currentEnd, $currentTotal);
    ...
    Insert_In_Section(\@fromSections, $c->{fromBeginToken}, $c->{fromToToken});
    Insert_In_Section(\@toSections, $c->{toBeginToken}, $c->{toToToken});
#    print "---\n";
#    Report_Cloned_Sections($prevToFile, \@toSections);
#    print "\n";
...
###        print join(';', $prevFromFile, $prevToFile, $currentTotal, $f1->{size}, $ratio), "\n";
###    }
###    
###}
###
