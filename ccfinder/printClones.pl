...
open(OUT, '>clonepairs.txt') || die("Unable to create clone pairs file");

foreach my $c (@$clonePairs) {
    next if ($c->{fromFile} == $c->{toFile});
#    next if ($c->{fromFile} > $c->{toFile});
#    next if ($c->{fromFile} == $c->{toFile} &&
#             $c->{fromBeginToken} > $c->{toBeginToken});
    printf(OUT "%d;" x 8 . "\n",
               $c->{fromFile}, 
               $c->{toFile}, 
               $c->{fromBeginToken},
...
open(OUT, '>files.txt') || die("UNable to create files ");
