...
//          console.log('updateA [' + tab.title + '] [' + filter_text  + ']');
          if (filter_text.length == 0 )
              return true;
//        return (tab.title.toLowerCase().indexOf(filter_text) !== -1) ||
//            (tab.url.toLowerCase().indexOf(filter_text) !== -1);
//          var re = /gmail/;
//          var matchTitle = tab.title.match(re);
          var re = new RegExp(filter_text,'i');
          var matchTitle = re.exec(tab.title);
//          console.log('updateAndDisplayMatchingTabs 2 [' + matchTitle  + ']');
          if (!matchTitle)
              matchTitle = re.exec(tab.url);
          return (matchTitle)
      };
