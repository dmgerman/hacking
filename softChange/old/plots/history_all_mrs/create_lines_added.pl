...
set key left
set xlabel "Date"
set ylabel "Lines added"
#set y2label "LOCS added in release"
#set arrow from "1998/01/01",0 to "2003/05/01",0  nohead lt 5 
#show arrow
plot '$TMP_FILE' using 1:2  title "Net lines added for $dbName" with lines
END
...
my $sth = $dbh->prepare(" 
                        select date_trunc('month', daterev) as d, sum(linesadd + linesrm) from revisions group by d order by d;
                        ");
