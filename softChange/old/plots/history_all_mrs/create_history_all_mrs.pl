...
my $TMP_FILE = "/tmp/create_history_all_mrs$$.data";

my $TMP_FILE_PLOT = "/tmp/create_history_all_mrs$$.gplot";
...
#show arrow
plot '$TMP_FILE' using 1:2  title "MRs for $dbName" with lines
END
...
my $sth = $dbh->prepare(" 
                        select date_trunc('month', daterev) as d, count(*) from mrs2 group by d;
                        ");
