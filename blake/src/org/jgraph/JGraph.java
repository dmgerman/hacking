 ...
 * <p>
 * JGraph supports in-place editing of text and shapes. These features can be
 * disabled using the setEnabled() method, which blocks all features, or
 * individually, using the following methods:
 * <p>
 * setEditable() controls in-place editing of cells. Moving, cloning, sizing,
 * and bending, connection and disconnection of edges may also be disabled using
 * the respective methods, namemly setMoveable(), setCloneable(), setSizeable(),
 * setBendable(), setConnectable() and setDisconnectable().
 * <p>
 * The model offers fainer control of connection establishment based on the
 * passed-in edge and port. The individual cells offer yet another level of
 * control in that they may allow/disallow being edited, moved, cloned, resized,
 * and shaped, or connected/disconnected to or from other cells.
 * <p>
...
// DO NOT REMOVE OR MODIFY THIS LINE!
	...
	public static final String VERSION = "JGraph (v5.1)";
	...
	/**
	 * True if inserted cells should be selected. Ignores selectionEnabled.
	 * Default is false.
	 */
	...
	public static void addSampleData(GraphModel model) {
		...
		attributes.put(gm, createBounds(new AttributeMap(), 20, 100,
				Color.blue));
		...
		attributes.put(cu, createBounds(new AttributeMap(), 280, 20,
				Color.red));
		cu.add(new DefaultPort("ComponentUI/Center"));
		DefaultGraphCell gu = new DefaultGraphCell("GraphUI");
		attributes.put(gu, createBounds(new AttributeMap(), 280, 100,
				Color.red));
	...
	/**
	 * Returns all <code>cells</code> including all descendants in the
	 * passed in order of cells.
	 */
	 ...
	 * more cells behind <code>current</code>. Note: This does only
	 ...
	 * Note: This returns the next <i>selectable</i> view.
	 * <br>
	...
	public CellView getNextViewAt(CellView current, double x, double y, boolean leafsOnly) {
		CellView[] cells = AbstractCellView.getDescendantViews(getGraphLayoutCache()
				.getRoots());
	 ...
	 * implementation returns the first floating port (ie. the first
	 * port that does not define an offset) or <b>the</b> port, if there
	 * is only one port.
	 * @param cell
	 * @return
	 */
	public PortView getDefaultPortForCell(Object cell) {
		if (cell != null && !getModel().isEdge(cell)) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				CellView child = getGraphLayoutCache().getMapping(
						childCell, false);
	 ...
	 * CellView or the current GraphLayoutCache returns a mapping for value,
	 * then then value attribute of that CellView is used. (The value is
	 * retrieved using getAllAttributes.) If the value is an instance of
	 * DefaultMutableTreeNode (e.g. DefaultGraphCell), then the userobject is
	 * returned as a String.
	 */
	public String convertValueToString(Object value) {
		CellView view = (value instanceof CellView) ? (CellView) value
	...
	public Point2D toScreen(Point2D p) {
		if (p == null)
			return null;
		p.setLocation(Math.round(p.getX() * scale), Math.round(p.getY())
				* scale);
		return p;
	}
	...
	public CellView createView(JGraph graph, CellMapper mapper, Object cell) {
		CellView view = null;
		if (graph.getModel().isPort(cell))
			view = createPortView(graph, mapper, cell);
		else if (graph.getModel().isEdge(cell))
			view = createEdgeView(graph, mapper, cell);
		else
			view = createVertexView(graph, mapper, cell);
		return view;
	}
	...
	public void updateAutoSize(JGraph graph, CellView view) {
		if (view != null && !graph.isEditing()) {
			if (GraphConstants.isAutoSize(view.getAllAttributes())) {
			...
			} else if (GraphConstants.isResize(view.getAllAttributes())) {
				AttributeMap attrs = graph.getModel().getAttributes(view.getCell());
				Rectangle2D bounds = GraphConstants.getBounds(attrs);
				if (bounds == null)
					bounds = GraphConstants.getBounds(view.getAttributes());
				if (bounds != null) {
					Dimension2D d = graph.getUI().getPreferredSize(graph, view);
					bounds.setFrame(bounds.getX(), bounds.getY(), d.getWidth(), d
							.getHeight());
					// Consume resize attribute
	 ...
	 */
	protected EdgeView createEdgeView(JGraph graph, CellMapper mapper,
			Object cell) {
		if (cell instanceof Edge)
			return createEdgeView(graph, mapper, (Edge) cell);
		else
	 ...
	 */
	protected PortView createPortView(JGraph graph, CellMapper mapper,
			Object cell) {
		if (cell instanceof Port)
			return createPortView(graph, mapper, (Port) cell);
		else
	 ...
	 * Constructs an EdgeView view for the specified object.
	 * 
	 * @deprecated replaced by {@link #createEdgeView(Object,CellMapper)}since
	 ...
	 * Constructs a PortView view for the specified object.
	 * 
	 * @deprecated replaced by {@link #createPortView(Object,CellMapper)}since
	 *             JGraph no longer exposes dependecies on GraphCell subclasses
	 *             (Port, Edge)
	 */
	 ...
	 */
	protected VertexView createVertexView(JGraph graph, CellMapper mapper,
			Object cell) {
	...
	/**
	 * Returns true if the graph is editable, ie. if it allows cells to be
	 * edited.
	 * 
	 ...
	 * editing begins to ensure that the given cell can be edited. This is
	 * provided as an entry point for subclassers to add filtered editing
	 * without having to resort to creating a new editor.
	 * 
