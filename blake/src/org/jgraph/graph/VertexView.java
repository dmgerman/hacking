...
public class VertexView extends AbstractCellView {
	...
	public static class SizeHandle implements CellHandle, Serializable {
		...
		public void mouseDragged(MouseEvent event) {
			...
			if (offscreen != null) {
				...
				if (t != null)
					dirty.add(t);
				int size = 2;
				dirty.setFrame(dirty.getX() - size - 1,
						dirty.getY() - size - 1, dirty.getWidth() + size * 2
								+ 1, dirty.getHeight() + size * 2 + 1);
		...
		protected Rectangle2D computeBounds(MouseEvent event) {
			...
			return vertex.getAttributes().createRect(left, top, width + 1,
					height + 1);
		}
