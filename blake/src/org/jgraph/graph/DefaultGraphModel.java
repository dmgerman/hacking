...
public class DefaultGraphModel
	...
	* In the new array, all references are replaced with references
	* to the cloned cells (ie parent or anchor). This method does
	* only include children which are in <code>cells</code>. Use
	* JGraph.getDescendantList to get a complete list of all
	* children.
	*/
	public Map cloneCells(Object[] cells) {
		...
		while (!q.isEmpty()) {
			...
			if (node instanceof DefaultGraphCell && !map.containsKey(node)) {
	 ...
	 * Note: The passed-in propertyMap may contain <code>PortView</code>s
	 * which must be turned into <code>Point</code>s when stored in the model.
	 */
	public void insert(
		Object[] roots,
		Map attributes,
		...
		if (attributes != null) {
			...
			while (it.hasNext()) {
				...
				if (cell instanceof GraphCell) {
					Map deltaOld =
						((GraphCell) cell).changeAttributes(deltaNew);
	...
	public class GraphModelEdit
		...
		protected void changeChildCount(
			Map childCount,
			Object parent,
			int change) {
			if (parent != null) {
	 ...
	 */
	public class GraphModelLayerEdit extends AbstractUndoableEdit
	...
	/**
	 * Returns the roots in <code>cells</code> by checking if
	 * their parent is <code>null</code>.
	 * This implementation only uses the GraphModel interface.
	...
	public static Set getEdges(GraphModel model, Object[] cells) {
		...
		List allCells = getDescendants(model, cells);
		if (allCells != null) {
